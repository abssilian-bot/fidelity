...
> **Inscription restaurateur — 9 septembre 2026 :** connexion par e-mail avec retour dans l’espace professionnel, dossier SIRET vérifié via l’Annuaire des entreprises et validation humaine par un administrateur avant rattachement du restaurant. Variable privée `FIDELITY_ADMIN_EMAILS` : liste d’adresses autorisées, promotion après validation d’un nouveau lien e-mail. Migration `20260909010000_restaurant_registration`. L’acceptation crée un programme inactif à configurer ; aucune permission de scan à la soumission. Lire [INSCRIPTION-RESTAURATEURS.md](INSCRIPTION-RESTAURATEURS.md) pour le parcours, les contrôles, les limites du SIRET et les 217 vérifications. Les paragraphes plus anciens ci-dessous décrivent aussi des étapes désormais dépassées.

> **Publication vérifiée le 9 septembre vers 18 h 49 :** `882bb3b` déployé sur [Fidelity](https://fidelity-api-mcld.onrender.com), migration et configuration administrateur appliquées. Les routes d’inscription et d’administration refusent les accès anonymes ; Wallet reste activé. Le rôle administrateur sera attribué au compte désigné lors de sa prochaine connexion par un nouveau lien e-mail.

> **Mise à jour du 8 septembre 2026 :** export Apple Wallet signé, protocole de mise à jour APNs et scan des cartes Wallet ajoutés. Le code `b21f5de` est déployé sur [le site public Fidelity](https://fidelity-api-mcld.onrender.com), les migrations ont réussi et les certificats sont installés dans les fichiers secrets Render. `/wallet/status` renvoie `enabled: true`. Lire [CARTES-WALLET.md](CARTES-WALLET.md) pour les variables `APPLE_WALLET_*`, le modèle de sécurité et les essais iPhone restant à effectuer. Les offres sont désormais persistées en JSON dans `Restaurant.offers`. Likes et abonnements des vrais comptes passent par `/social/…`. Lire [PREPARATION-APPLE.md](PREPARATION-APPLE.md) : le projet reste un site web, plusieurs prérequis de soumission iOS sont encore absents. Aucune garantie d'acceptation Apple.
