  ...
  async function request(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (intent === 'restaurant') {
        const destination = new URL(window.location.href); destination.searchParams.set('espace', 'restaurant')
        window.history.replaceState(window.history.state, '', destination.pathname + destination.search)
      }
      const result = await requestLoginLink(email, intent)
    ...
    <section className="form-section account-section" aria-labelledby="account-heading">
      ...
      {account ? (
        ...
        <form onSubmit={(event) => void request(event)} className="account-form">
          <p className="muted" style={{ margin: 0, fontSize: 14, lineHeight: 1.6 }}>{intent === 'restaurant' ? 'Déjà inscrit ou nouveau restaurateur : entrez votre e-mail, puis ouvrez le lien reçu. Votre compte est créé à la première connexion. Aucun mot de passe ni code restaurant à saisir.' : 'Ton e-mail suffit pour créer ton compte ou te reconnecter. Aucun mot de passe à retenir.'}</p>
