...
1. Créer une application iOS, la compiler et la signer avec Xcode, la tester sur iPhone et préparer la fiche App Store Connect. La simple disponibilité du site ne constitue pas une soumission iOS. *(10 sept. : projet iOS Capacitor généré dans `app/ios/`, voir TRANSFERT-MAC.md — reste la compilation sur Mac.)*
2. ~~Ajouter une suppression de compte dans l'application~~ **Fait le 10 septembre** : `DELETE /auth/me` efface le compte et toutes les données personnelles en une transaction (adhésions, points, avis, publications, likes, follows, favoris, passes Wallet — le ledger des opérations validées est conservé mais détaché de l'opérateur). Refus 409 si le compte possède un établissement. Bouton en 2 étapes dans Réglages → Compte. Couvert par 3 tests (suppression complète + token rejeté, 409 restaurateur, 401 sans session).
...
4. Publier une politique de confidentialité et un contact d'assistance réels, avec l'identité de l'éditeur et les traitements/durées de conservation confirmés. Renseigner correctement les déclarations de données et les permissions caméra dans la version iOS. *(10 sept. : `NSCameraUsageDescription` et `ITSAppUsesNonExemptEncryption` déjà déclarés dans l'Info.plist.)*
